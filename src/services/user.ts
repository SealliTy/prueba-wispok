...
import { IUser, IWeather } from '../types/user';
import { createAsyncThunk } from '@reduxjs/toolkit';

const apiKey = import.meta.env.VITE_API_KEY_WATHER
  ...
  async (user: IUser, thunkAPI) => {
    try {
      const data = await axios.get(
        `https://api.openweathermap.org/data/2.5/weather?lat=${user.lat}&lon=${user.long}&appid=${apiKey}&lang=es&units=metric`
      );
      ...
      return thunkAPI.rejectWithValue('Ocurrio un error');
    }
  }
);